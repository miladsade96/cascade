...
final class PartitionLogSuite extends FunSuite:
  test("retains one deterministic latest batch for every active producer") {
    val directory = Files.createTempDirectory("cascade-producer-state-test")
    try
      val log = PartitionLog(directory, maxSegmentBytes = 1024)
      try
        log.append(TestRecordBatch.producer(9L, 1, 0))
        log.append(TestRecordBatch.producer(3L, 2, 0))
        log.append(TestRecordBatch.producer(9L, 1, 1))
        assertEquals(
          log.latestProducerBatches.map(value => (value.producerId, value.producerEpoch, value.lastSequence)),
          Vector((3L, 2.toShort, 0), (9L, 1.toShort, 1))
        )
      finally log.close()

      val recovered = PartitionLog(directory, maxSegmentBytes = 1024)
      try
        assertEquals(recovered.latestProducerBatches.map(value => value.producerId -> value.lastSequence), Vector(3L -> 0, 9L -> 1))
