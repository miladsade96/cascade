...
private object OperationsJson:
  ...
  def status(snapshot: BrokerMetricsSnapshot, health: BrokerHealth): String =
    s"""{"node_id":${snapshot.nodeId},"uptime_ms":${snapshot.uptimeMillis},"running":${snapshot.running},"ready":${health.ready},"clustered":${snapshot.clustered},"controller_id":${snapshot.controllerId},"broker_fenced":${snapshot.brokerFenced},"topics":${snapshot.topics},"partitions":${snapshot.partitions},"active_connections":${snapshot.activeConnections},"inflight_requests":${snapshot.activeRequests},"pending_flush_bytes":${snapshot.pendingFlushBytes},"usable_disk_bytes":${snapshot.usableDiskBytes},"checks":${checks(health)}}"""
