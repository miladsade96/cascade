...
object MetadataCodec:
  ...
  def minimumRequiredFormat(metadata: ClusterMetadata): Short =
    if metadata.featureLevels.contains(ClusterFeature.IndependentCoordinator) then 12
    else if metadata.featureLevels.contains(ClusterFeature.ShardObjectStorage) then 11
