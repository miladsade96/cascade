...
So far, I've built durable segmented storage, Kafka magic-v2 record batches, broker-assigned offsets, idempotent producers, transactions, `read_committed` isolation, classic consumer groups, ISR replication, leader promotion, online partition reassignment, dynamic quorum membership with broker fencing, storage lifecycle management, client-listener security, hostname-verified mutual TLS and certificate-bound identities between brokers, resource isolation, Prometheus metrics, health/readiness, structured events, capacity alerts, Kafka Admin configuration reads, and checksummed offline backup/restore.

The clean test suite passes **200/200 unit, integration, end-to-end, and fault-qualification tests**. I test with the real Kafka 4.3.1 Java client and cover restart recovery, persisted high watermarks, incremental replica repair, transaction commit and abort, transactional consumer offsets, group rebalancing, encrypted peer replication and quorum failover, certificate impersonation/trust/hostname rejection, online reassignment, dynamic voter changes, TLS/SASL and ACLs, low-disk rejection, operations endpoints, Admin `DescribeConfigs`, and exact Kafka-visible recovery from a restored backup.

I also reran my documented 10-million-record load test. Cascade produced **350,319 records/s (342.1 MiB/s)**, consumed **329,303 records/s (321.6 MiB/s)**, and verified all **10,000,000 records without gaps** on my development machine. I published the setup and full result in the repository. This is a local single-node benchmark, not a production capacity claim.

I want to be honest about where the project is today: Cascade isn't a production Kafka replacement yet. It still needs more SASL mechanisms, TLS key-store hot reload, Kafka ACL Admin APIs, the new consumer protocol, per-topic lifecycle policy, rolling-version qualification, multiple client-language matrices, physical power/device-loss tests, multi-day soak tests, and coordinator scale work. I'm publishing it now because the foundation works, and I want to build the remaining gates in the open.
...
- TLS/SASL, broker mTLS identities, ACLs, quotas, retention/compaction, Prometheus metrics, and health/readiness
- Checksummed offline backup/restore with exact Kafka-client recovery
- 200/200 automated tests
- 10 million records verified without gaps
- 350K produced and 329K consumed records/s in my documented development-machine test
