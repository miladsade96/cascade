...
After adding Kafka-compatible SCRAM-SHA-256/512, offline verifiers, live atomic rotation, strict exchange limits, audit/metrics, and credential-policy readiness, I ran a clean **221/221** suite and repeated the full ten-million workload on 2026-08-27. It consumed exactly **10,000,000 / 10,000,000** records at **333,835 produced records/s** (**326.0 MiB/s**) and **349,851 consumed records/s** (**341.7 MiB/s**). Produce took 29.955 seconds and consume took 28.584 seconds. Acknowledgement p50/p95/p99/p99.9 fell in the harness's `<=5000 ms` bucket and maximum latency was 3,989.008 ms. Produce used 4.02 CPU cores and 33.5% machine CPU with 71 collections and 2,961 ms of GC; consume used 3.07 cores and 25.5% machine CPU with 413 collections and 866 ms of GC. Peak heap was 5,387.4 MiB. The run stored 9,862.3 MiB, forced 9,675.9 MiB in 247 operations over 10,183.2 ms, and left 186.4 MiB pending for clean shutdown. The harness is single-node plaintext with authentication disabled, so this qualifies exactness and the inactive-authentication data path; it does not measure SCRAM, TLS, or replicated-cluster capacity.

After adding Kafka-compatible OAUTHBEARER, signed OAuth/OIDC JWT policy, strict JSON/JWKS limits, verified HTTPS refresh with ETags, live signing-key rotation, expiring identities, and operational integration, I finished with **244/244** passing tests and repeated the full ten-million workload on 2026-08-30. It consumed exactly **10,000,000 / 10,000,000** records at **368,387 produced records/s** (**359.8 MiB/s**) and **351,784 consumed records/s** (**343.5 MiB/s**). Produce took 27.145 seconds and consume took 28.427 seconds. Acknowledgement p50/p95/p99/p99.9 fell in the harness's `<=5000 ms` bucket and maximum latency was 3,213.600 ms. Produce used 4.19 CPU cores and 34.9% machine CPU with 65 collections and 2,719 ms of GC; consume used 2.87 cores and 23.9% machine CPU with 401 collections and 976 ms of GC. Peak heap was 5,391.9 MiB. The run stored 9,862.3 MiB, forced 9,710.2 MiB in 221 operations over 9,803.9 ms, and left 152.1 MiB pending for clean shutdown. The harness is single-node plaintext with authentication disabled, so this qualifies exactness and the inactive-OAuth data path; it does not measure OAuth, TLS, or replicated-cluster capacity.
