...
- Peer capability exchange, a quorum-committed feature-level image, metadata-format negotiation through format 8, and a committed compatibility floor that survives controller loss while prospective voters still require a live compatibility check.
...
| Consumer groups | New consumer protocol, static-member fencing, administrative group APIs, offset retention and compaction, and coordinator failover | Classic groups plus `ConsumerGroupHeartbeat` v0, server-side assignment, member epochs, deterministic topic IDs, static fencing, expiry/compaction, rendezvous sharding, and coordinator failover pass; administrative group APIs, later heartbeat versions, and high-cardinality scale evidence remain |
...
I completed the automatable feature-negotiation, coordinator-sharding, consumer-heartbeat-v0, online local snapshot, advanced-compaction, conservative distributed-quota, qualification-harness, and pinned rolling-binary work in this milestone. The complete suite passes **308/308** tests, and the separate 1.0.0-to-1.1.0 campaign passes upgrade, pre-activation rollback, activation, safe downgrade rejection, majority traffic, recovery, and exact 40/40 consumption. Java, JavaScript, Python, Go, and .NET clients cross the built container with exact records, and Kafka 4.3.1 exercises both classic and server-assigned consumer protocols. The latest one-million regression consumed exactly **1,000,000 / 1,000,000** records at **396,006 produced records/s** and **186,386 consumed records/s**; the latest ten-million default-path regression remains **10,000,000 / 10,000,000**. I will not call the project production ready until the unexecuted multi-day, physical-hardware, arbitrary-network, restore-drill, and dedicated-host gates above have archived evidence.
