...
| Metadata | Durable quorum controller, fencing, leader epochs, reassignment, and cluster membership changes | Durable election, leases, fencing, metadata images, leader epochs, resumable reassignment, joint consensus, deterministic partition recovery, dual-majority safety, peer capability exchange, and quorum-committed feature levels pass; an archived old/new binary upgrade-downgrade matrix and exhaustive external failure qualification remain |
...
| Compatibility | Supported-version contract, malformed-frame and fuzz tests, multiple client languages, and a rolling upgrade and downgrade matrix | Exact API ranges, a non-regression contract, fuzzing, Java 4.3.1, KafkaJS 2.2.4, Python 2.15.0, franz-go 1.21.0, and .NET 2.15.0 pass; broader client versions and mixed broker-version upgrade/downgrade tests remain |
...
I completed the automatable feature-negotiation, coordinator-sharding, consumer-heartbeat-v0, online local snapshot, advanced-compaction, conservative distributed-quota, and qualification-harness work in this milestone. The complete suite passes **307/307** tests in **2 minutes 8 seconds** on 2026-08-31. Java, JavaScript, Python, Go, and .NET clients cross the built container with exact records, and Kafka 4.3.1 now exercises both classic and server-assigned consumer protocols. The latest one-million regression consumed exactly **1,000,000 / 1,000,000** records at **396,006 produced records/s** and **186,386 consumed records/s**; the latest ten-million default-path regression remains **10,000,000 / 10,000,000**. I will not call the project production ready until the unexecuted mixed-binary, multi-day, physical-hardware, and dedicated-host gates above have archived evidence.
