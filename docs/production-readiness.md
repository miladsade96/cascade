...
I completed the client security and resource-isolation milestone: TLS, Kafka SASL/PLAIN, salted reloadable credentials, deny-by-default reloadable ACLs, forced audit events, connection/request admission, principal ingress quotas, and overload shedding. My next milestone is operations: metrics, health/readiness, structured logs, backup/restore, capacity signals, and runbooks. I will also keep security hardening in scope because clustered deployments still need authenticated/encrypted peer traffic, certificate hot reload, additional SASL mechanisms, Kafka ACL Admin APIs, egress quotas, and multi-tenant soak qualification.
