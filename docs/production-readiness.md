...
- Global and per-IP connection caps, a bounded in-flight request semaphore, immediate overload shedding, and isolated per-principal ingress, egress, Produce, and Fetch token buckets with bounded Kafka throttle fields.
...
| Storage lifecycle | Time and size retention, log and offset compaction, timestamp and transaction indexes, disk-pressure handling, and safe deletion | Quorum-committed per-topic retention/cleanup, conservative uncompressed whole-batch key compaction, rebuilt indexes, offset expiry, bounded journals, atomic replacement recovery, and low-disk rejection pass; compressed-record compaction, tombstone grace, deletion throttling, replicated cleanup coordination, and external disk-failure qualification remain |
...
| Resource isolation | Client and user quotas, request and connection limits, bounded queues, overload shedding, and multi-tenant tests | Global/per-IP connection caps, bounded in-flight requests, independent broker-local ingress/egress/Produce/Fetch principal quotas, request frame bounds, Kafka throttle fields, and immediate shedding pass; distributed quota accounting and long multi-tenant soak tests remain |
| Operations | Metrics, health and readiness endpoints, structured logs, admin API coverage, backup and restore, container/orchestrator deployment, and capacity alerts | Broker metrics/health/status, rotating events, capacity signals, topic configuration mutation, offline checksummed backup/restore, a non-root distroless image, hardened Compose/Kubernetes, ServiceMonitor/Prometheus/Grafana assets, publishing automation, runbooks, and exact recovery tests pass; an external TLS/mTLS operations boundary, online cluster snapshots, scheduled retention, and recurring restore drills remain |
...
| Consumer groups | New consumer protocol, static-member fencing, administrative group APIs, offset retention and compaction, and coordinator failover | Classic generations/members/assignments/offsets, duplicate static-member fencing, expiry/compaction, and elected-coordinator failover pass; the new protocol, administrative group APIs, sharding, and scale qualification remain |
...
I completed the automatable security, local quota, per-topic policy, static-member, malformed-frame, deployment, and client-language gates in this milestone. The clean suite passes **290/290** tests. Java, JavaScript, Python, Go, and .NET clients cross the built container with exact records, KafkaJS offset commits/fetches use their real v5/v4 shapes, and restart recovery preserves the original records. The latest one-million regression consumed exactly **1,000,000 / 1,000,000** records; the latest ten-million default-path regression remains **10,000,000 / 10,000,000**. I will not call the project production ready until the remaining rows above have their external and mixed-version qualification evidence.
