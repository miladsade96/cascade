...
I completed atomic TLS key/trust-store reload after the OAuth/OIDC and production-container milestones. New client handshakes use one validated generation, established sessions continue, peer channels reconnect on generation changes, and malformed replacements preserve the last valid context while failing readiness. The clean suite passes **255/255** tests. It includes real Kafka listener key/trust replacement and an RF=3 old-CA → overlap → new-leaf → new-CA sequence with exact traffic plus controller failover. The container smoke still passes exact idempotent Kafka traffic before and after restart, and my latest ten-million-record default-path regression consumed exactly **10,000,000 / 10,000,000** records. My next security work is Kafka ACL Admin APIs, claim-to-role policy, broader OAuth keys/tokens, egress/distributed quotas, and multi-tenant soak qualification.
