...
| Compatibility | Supported-version contract, malformed-frame and fuzz tests, multiple client languages, and a rolling upgrade and downgrade matrix | Exact API ranges, a non-regression contract, fuzzing, Java 4.3.1, KafkaJS 2.2.4, Python 2.15.0, franz-go 1.21.0, .NET 2.15.0, and real 1.0.0/1.1.0 broker processes pass; broader client versions and every future adjacent release pair remain |
...
The complete Scala/Kafka-Java suite now passes **335/335** tests. The coordinator-delta campaign commits **3,000 writes across 1,000 groups**, exercises every broker, and verifies exact offsets before/after controller failover and after restarting all brokers. Its proposed delta payloads are **98.2% smaller** than equivalent full images, but its **30.455 writes/s** and **4,011.691 ms p99** are not a production-capacity pass: client churn approached this Windows host's narrow ephemeral-port range, and the full-image quorum architecture still limits scaling. I keep the [measured report and limitations](performance/2026-09-02-coordinator-scale.md) alongside the [coordinator runbook](coordinator-scaling.md).

The latest one-million default-path regression consumed exactly **1,000,000 / 1,000,000** records at **482,015 produced records/s** and **497,901 consumed records/s**. The earlier ten-million and external JavaScript/Python/Go/.NET matrix results remain historical evidence; this milestone did not rerun those external language/container jobs. I will not call the project production ready until the unexecuted multi-day, physical-hardware, arbitrary-network, restore-drill, independent-coordinator-capacity, and dedicated-host gates above have archived evidence.
