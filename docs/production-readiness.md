...
- TLS 1.2/1.3 listeners with PKCS12/JKS keys, optional client-certificate verification, and Kafka-compatible `SASL_SSL` or `SASL_PLAINTEXT` authentication using PLAIN, SCRAM-SHA-256, or SCRAM-SHA-512.
...
- Offline salted SCRAM verifiers and salted PBKDF2-SHA-256 PLAIN credentials, constant-time verification, bounded exchanges, connection-scoped principals, mechanism-aware audit/metrics, and atomic credential rotation that preserves the last valid snapshot and gates readiness on malformed replacements.
...
- Real Kafka 4.3.1 `SASL_SSL` Admin/Producer/classic-group Consumer tests for SCRAM-SHA-256 and SCRAM-SHA-512 that cross authentication, verifier rotation, ACL, storage, offset commit, TLS trust, metrics, readiness, and audit paths.
...
| Security | TLS, SASL mechanisms, authorization and ACLs, audit events, and secret rotation | Client TLS, hostname-verified peer mTLS, certificate-bound nodes, Kafka PLAIN and SCRAM-SHA-256/512, offline verifiers, resource ACLs, forced JSONL audit, and live identity/credential/ACL policy rotation pass; OAuth/OIDC, TLS key-store hot reload, Kafka ACL Admin APIs, and multi-tenant qualification remain |
...
I completed the SCRAM authentication milestone: Kafka-compatible SHA-256/512 exchanges, offline verifiers, unknown-user work equalization, strict protocol bounds, atomic rotation, mechanism-aware audit/metrics, readiness integration, and real Kafka client coverage over TLS. The full suite passes **221/221** tests, and my ten-million-record default-path regression consumed exactly **10,000,000 / 10,000,000** records. My next security work is OAuth/OIDC, TLS key/trust-store hot reload, Kafka ACL Admin APIs, egress/distributed quotas, and multi-tenant soak qualification.
