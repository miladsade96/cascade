...
| --- | --- | --- |
| Availability | At least three brokers, replicated partitions, ISR tracking, leader election, and verified recovery from broker, process, and disk loss | Graceful partition-leader and controller failover plus returning-replica re-admission pass; crash/power-loss qualification, persisted high watermarks, and incremental recovery remain |
...
My next availability milestone is persisted partition high-watermark recovery and incremental segment transfer. I also need to replicate group, offset, producer, and transaction coordinator state before controller failover can preserve those services.
