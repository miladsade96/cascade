...
- Offset mutation and generation validation use the same write-side critical section. Readers use an immutable acknowledged key/group index, so a value staged for checkpointing remains invisible without making `OffsetFetch` wait for publication.
- OffsetFetch captures readiness once and then captures one acknowledged view for all requested offsets. It reuses that readiness decision for every response error field; a readiness transition cannot turn an unavailable lookup into a successful missing-offset response.
...
Unit tests cover bounds, FIFO rewinds, isolated invalid commands, rollback, acknowledged-view isolation, ownership/static-member revalidation, byte splitting, timeout cancellation, interruption, worker exceptions, shutdown drain, and metrics. A real three-broker test pauses a non-controller owner's publication RPC and requires a wire-level `OffsetFetch` to return the previous acknowledged offset before the pause is released. Another quorum test rejects a batched write under quorum loss, retries, and verifies exact restart recovery. Persistent Kafka-client end-to-end runs compare batching with a single-request control through controller loss and full restart. CI additionally runs 1,000-group persistent controls at 8/32 workers and an 8-worker churn campaign, archiving exact offsets, latency, admission, read-view, and coalescing evidence without asserting hardware-independent throughput.

The [acknowledged read-isolation design](coordinator-read-isolation.md) removes the publication monitor from committed-offset reads, but it does not change service write serialization. The later [format-12 coordinator architecture](coordinator-architecture.md) adds per-shard quorum publication. Distributed decision resolution, shard-journal compaction, finer-grained mutation locks, less whole-state CPU work, large membership/transaction churn, and dedicated-host RF=3 qualification remain release gates.

The subsequent [snapshot preparation work](coordinator-snapshots.md) reduces encoding and repeated installation costs without changing this queue's admission, cancellation, or atomic rollback contract. It adds four node-only preparation counters and ownership-aware group expiry; batching itself does not renew consumer sessions. I compare the preparation counters with checkpoint failures and end-to-end latency rather than treating cache reuse as an SLO.
...
If the default Go module proxy is unavailable, `-GoProxy direct` downloads from upstream repositories without disabling pinned `go.sum` verification or TLS. The [2026-09-03 qualification report](performance/2026-09-03-offset-batching.md) includes matched controls, the fetch defect found during testing, Linux and external-client results, and the remaining release gates.
