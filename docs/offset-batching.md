...
The [acknowledged read-isolation design](coordinator-read-isolation.md) removes the publication monitor from committed-offset reads, but it does not change write serialization. Independent per-shard consensus/publication, finer-grained mutation locks, less whole-state CPU work, large membership/transaction churn, and dedicated-host RF=3 qualification remain release gates.
