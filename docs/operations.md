...
For [offset batching](offset-batching.md) I compare rates of `cascade_offset_batch_requests_total` and `cascade_offset_batches_total`, alongside pending/peak request and estimated-byte gauges. `cascade_offset_batch_rejected_total` counts pre-admission refusal, while `cascade_offset_batch_failed_total` counts admitted commands ending in errors. My Prometheus rules warn on sustained admission rejection and an admitted failure ratio above ten percent. These rules are inert on older images that do not export the new metrics; deployment image pins remain unchanged. The ratio includes expected retriable conflicts/fencing, so I investigate the cause rather than automatically restarting brokers. The metric surface now has 99 fixed series per broker with the currently configured mechanism/quota labels.
