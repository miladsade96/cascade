...
Most metric series have only the `node_id` label. SASL success and failure counters add one bounded `mechanism` label with `PLAIN`, `SCRAM-SHA-256`, `SCRAM-SHA-512`, `OAUTHBEARER`, or `UNKNOWN`. Traffic quota metrics use bounded direction/class labels rather than principal names. I export broker uptime/fencing/controller state, topic and local-partition counts, connection/request admission, request and response traffic, cumulative request duration/failures, client and peer authentication/TLS/rejection totals, TLS material generation/reload/failure state, ingress/egress/Produce/Fetch quota activity, distributed quota reservations/forwarding/failures/epoch resets, flush work/backlog, storage lifecycle/rejection totals, disk capacity, and JVM heap. I keep topic, client, principal, key ID, token, and request identifiers out of labels to avoid unbounded cardinality and secret leakage.

For [distributed quotas](distributed-quotas.md), I expect reservations, allowed decisions, throttles, and rejections on the active controller. Followers increment the forwarding counter. A configuration mismatch or coordination failure is fail-closed and needs immediate investigation. An epoch reset is normal after controller election, but repeated resets indicate controller instability; the replacement ledger deliberately starts empty and earns capacity at the configured rate.
...
For [acknowledged coordinator reads](coordinator-read-isolation.md) I graph offset-view captures and returned keys separately from stable-offset and transaction-visibility decisions. A flat counter under known traffic means the request bypassed the intended indexed view or metrics collection stopped. A high key-to-snapshot ratio describes larger OffsetFetch responses; it is not an error by itself. I correlate read rates with publication latency to confirm that publication pressure does not stop acknowledged reads.

For the [independent coordinator quorum](coordinator-architecture.md) I alert on `cascade_coordinator_quorum_failed_total`, `cascade_coordinator_quorum_rejected_total`, and a nonzero `cascade_coordinator_shard_transactions_pending` that does not clear. I compare `cascade_coordinator_quorum_inflight` and `cascade_coordinator_quorum_peak_inflight` with the configured admission limit, and use phase/message/record-byte counters to explain amplification. `cascade_coordinator_shard_journal_bytes` is a storage-budget gauge; force time and torn-tail truncation are durability signals. These metrics have only the bounded `node_id` label.

For [consumer group administration](group-administration.md) I compare listing snapshots with returned entries, descriptions with hits, and deletion attempts with failures. A failure is not automatically an incident: an active group, denied ACL, coordinator move, or rejected checkpoint can all be expected. I investigate the error mix and coordinator health before retrying. These six counters use only `node_id`; the complete metric fixture now has 164 fixed series per broker with the configured mechanism/quota labels.

I run the repeatable [coordinator scale qualification](coordinator-architecture.md) before release. CI archives its JSON evidence for 1,000 groups, 32 concurrent workers, controller failover, and complete broker restart. I require zero quorum failure and admission rejection, then investigate sustained Kafka retry ratios, force-time growth, or checkpoint latency growth before increasing tenant cardinality. I do not use development-machine throughput as a production SLO.
...
Kafka Admin 4.3.1 can also list, describe, and delete consumer groups through the explicit versions in the README. I verify an empty group before deletion, treat `NON_EMPTY_GROUP` as a safety result, and re-describe after an ambiguous timeout. Group visibility and deletion require the separate ACLs described in the [group administration runbook](group-administration.md).
