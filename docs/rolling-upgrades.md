...
The current source release is `1.7.0`; its [release notes](releases/1.7.0.md) record source qualification and local-image status separately from Docker Hub publication. Reclaimable quotas remain on the conservative level-1 path until every voter advertises and commits `distributed-quotas` level 2. `-Baseline format10` pins the 1.2.0 source at `fbe4e98a7b9c55680a365453041ee084c0a6efbf`; its existing features remain active during a mixed cluster, while `shard-object-storage` must remain inactive. Unanimous activation raises the floor to format 11. Backup/restore and offline inspection must include the sibling shard-object directory.
