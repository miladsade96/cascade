...
This closes the broker-to-broker encryption and authentication gate. SCRAM-SHA-256/512 is available for client authentication, but it does not replace certificate-bound peer identity. I still need TLS key-store hot reload, OAuth/OIDC, Kafka ACL Admin APIs, and a rolling-version protocol negotiation layer, which I track in my [production-readiness checklist](production-readiness.md).
