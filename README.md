...
The current release is `1.6.0`. Broker build metadata, Docker labels, the compatibility package, and deployment manifests use the same version. This source release completes the delivery-semantics surface described in the [1.6.0 release notes](docs/releases/1.6.0.md); its Docker Hub tag is not published.
