...
> [!IMPORTANT]
> Cascade isn't a production Kafka replacement yet. The code has peer capability negotiation, a negotiated metadata-format floor, coordinator sharding with incremental replication, immutable shard-object persistence, bounded cross-owner publication batching, Kafka's ConsumerGroupHeartbeat v0 path, and acknowledged consumer-group administration. Metadata v4-v12 discovery with v10-v12 topic IDs, online point-in-time broker snapshots, record-level/gzip compaction, conservative cluster-shared quotas, and repeatable soak/power-loss probes are also implemented. Pinned 1.0.0, format-9, and format-10 source rolling campaigns against 1.3.0-SNAPSHOT pass, including pre-activation rollback and unsafe-downgrade rejection. The published format-8 image pair, multi-day soak, and physical power/device-loss campaign remain unqualified. Independent coordinator consensus and service locks remain open. A cluster-wide snapshot still needs one artifact from every replica host, and compaction for Snappy/LZ4/Zstd batches is still conservative.

## Performance I measured

The latest coordinator increment adds [bounded controller publication batching](docs/coordinator-publication.md) on top of [content-validated snapshot caching](docs/coordinator-snapshots.md) and [bounded offset batching](docs/offset-batching.md). Compatible proposals from different owners share a quorum publication; stale or malformed proposals fail independently, and retained work is bounded. A 1,000-group campaign preserved 3,000 acknowledged writes and every final offset through controller failover and full restart, combining 1,675 proposals into 1,156 batches without queue rejection. Its 57.934 writes/s and 4.715-second p99 are correctness evidence, not production capacity or a controlled comparison. Shared consensus, service locks, remaining full-state work, and external qualification remain gates.
...
The most recent complete Scala suite passes **475/475 tests**, with six additional read-isolation cases passing in focused runs. [Acknowledged coordinator read isolation](docs/coordinator-read-isolation.md) gives `OffsetFetch` and `read_committed` Fetch one immutable acknowledged view per response, without waiting on the publication monitor or exposing staged state. The latest [1,000-group campaign](docs/performance/2026-09-07-coordinator-read-isolation.md) verified **3,000 writes, 3,000 acknowledged offset snapshots, and 1,000/1,000 final offsets** through controller loss and full restart, with zero connection, offset-batch, or publication admission rejection. Its 104.495 writes/s is development-machine evidence under different settings, not a claimed speedup or production SLO. The latest million-record regression verified every record at **493,225 produced records/s** and **489,511 consumed records/s**. Linux storage/batching previously passed **51/51** tests, and all five external languages verify 25/25 records, with Java restart recovery. The shared metadata quorum, write-side locks, and full-state CPU work still prevent a production-capacity claim. Every advertised Metadata version (v4-v12) remains schema-tested, and anonymous idempotent producers bootstrap through every broker. The separate external-language matrix remains a CI gate:
...
| ---: | --- | --- |
| 1 | Coordinator capacity | Bounded cross-owner publication and immutable acknowledged read views are implemented; independent per-shard consensus beyond the shared metadata quorum, finer-grained mutation locks, remaining full-state CPU work, membership/transaction churn at scale, and dedicated-host capacity qualification remain |
...
- The controller now [batches compatible shard proposals](docs/coordinator-publication.md) from different owners into one bounded quorum publication. FIFO validation rejects stale or malformed proposals independently and preserves atomic multi-shard updates. This reduces redundant shared-quorum rounds; it is not independent shard consensus or execution.
- OffsetFetch and `read_committed` decisions now use [immutable acknowledged views](docs/coordinator-read-isolation.md). They stay on the old complete view during a tentative publication and switch atomically after acknowledgement. Group, transaction, checkpoint, and metadata mutation still use shared write-side coordination.
