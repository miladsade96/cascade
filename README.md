...
The latest coordinator increment adds [content-validated snapshot caching](docs/coordinator-snapshots.md) on top of [bounded offset batching](docs/offset-batching.md). It avoids full candidate serialization, reuses unchanged shard bytes, and fixes session renewal and blocked group waiters during image installation. Group expiry now runs on the ready assigned coordinator. A paired preparation benchmark reduced allocation by about **75%**; that is not a broker-throughput claim. New 1,000-group campaigns preserved exact offsets through failover and full restart at **41.249 writes/s** with 8 workers and **97.773 writes/s** with 32 workers. The latter still has **2.303-second p99** and significant retried failures, so I do not call it production capacity. Shared consensus, service locks, remaining full-state work, and external qualification remain gates.
