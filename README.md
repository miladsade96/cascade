...
The most recent complete Scala suite passes **498/498 tests**. [Acknowledged coordinator read isolation](docs/coordinator-read-isolation.md) now covers `OffsetFetch`, group administration, and `read_committed` Fetch with immutable acknowledged views that do not wait on tentative publication. The latest [1,000-group campaign](docs/performance/2026-09-08-group-administration.md) verified **3,000 writes, 3,002 acknowledged offset snapshots, and 1,000/1,000 final offsets** through controller loss and full restart, with zero connection, offset-batch, or publication admission rejection. Its 68.060 writes/s and 3,503.028 ms p99 are development-machine evidence, not a production SLO. The latest million-record regression verified every record at **493,225 produced records/s** and **489,511 consumed records/s**. Linux storage/batching previously passed **51/51** tests, and all five external languages verify 25/25 records, with Java restart recovery. The shared metadata quorum, write-side locks, and full-state CPU work still prevent a production-capacity claim. Every advertised Metadata version (v4-v12) remains schema-tested, and anonymous idempotent producers bootstrap through every broker. The separate external-language matrix remains a CI gate:
