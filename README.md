...
The current release is `1.3.1`. Broker build metadata, Docker labels, the compatibility package, and deployment examples use the same version. This security patch replaces the Debian/glibc/OpenSSL base with a minimal, pinned Alpine/musl runtime and makes image vulnerability scanning a blocking release gate. Broker features and format 11 remain unchanged from 1.3.0. I record qualification, publication status, and the exact Linux/amd64 image digest in the [1.3.1 release notes](docs/releases/1.3.1.md). This release does not change `latest` or older image tags.

`miladsade96/cascade:1.3.1` is published. Its local and Docker Hub registry scans on 2026-09-04 returned zero vulnerability findings at every severity, with no exceptions. I retain the dated reports and test limitations in the release notes; a clean scan is not a permanent security guarantee.
...
> [!IMPORTANT]
> Cascade isn't a production Kafka replacement yet. The code has peer capability negotiation, a negotiated metadata-format floor, coordinator sharding, incremental replication, immutable shard-object persistence, and a format-12 coordinator quorum that moves steady-state group, offset, producer, and transaction writes off the metadata-controller log into forced per-shard journals. Kafka's ConsumerGroupHeartbeat v0 path and acknowledged consumer-group administration are also implemented. Pinned source rolling campaigns through format 11 pass, but the format-11-to-format-12 and published-image campaigns remain open. Broker-local group/delivery mutation still shares one lock, distributed resolution after a majority decision is incomplete, and shard-journal checkpoint compaction still needs implementation. The multi-day soak, physical power/device-loss campaign, coordinated cluster snapshot, and Snappy/LZ4/Zstd compaction also remain open.

## Performance I measured

The latest coordinator increment adds an [independent coordinator architecture](docs/coordinator-architecture.md) on top of typed sharding, content-validated snapshot caching, and bounded offset batching. A fresh 1,000-group campaign preserved 3,000 acknowledged writes and every final offset through controller failover and full restart. It committed 672/672 shard-quorum transactions with no failed or rejected transaction, batch failure, or torn journal tail. The measured phase reached **368.719 writes/s** with **256.974 ms p99** on my Windows development host. This is a correctness and regression result, not a dedicated-host capacity limit or a controlled comparison with the older shared-quorum runs.
...
- Atomic coordinator changes replicated either through the mixed-version metadata path or, at format 12, through forced per-shard quorum journals and monotonic shard-image installation.
...
The configured node list bootstraps the first committed voter set and gives observers discovery endpoints. After that, the committed metadata image is authoritative. Recovery is incremental at Kafka batch boundaries; only its final delta and ISR admission are partition-fenced. Coordinator mutations are acknowledged only after the owner and a voter majority force the terminal transaction record. I route typed keys with rendezvous hashing, isolate conflicts across 64 group shards, 64 transaction shards, and one allocator shard, and merge independently advancing images by shard version. Formats 9–11 retain the older controller-publication paths for rolling compatibility. Format 12 uses the [independent coordinator quorum](docs/coordinator-architecture.md); broker-local service mutation, unresolved decided transactions after process loss, journal checkpointing, and dedicated-host capacity remain explicit boundaries.
...
The most recent complete Scala suite passes **525/525 tests**. The focused independent-coordinator matrix passes **81/81**, including record framing, torn tails, multi-shard recovery, majority loss, disjoint-shard concurrency, failover/restart, read isolation, and metrics. The latest [1,000-group campaign](docs/performance/2026-09-11-independent-coordinator.md) verified **3,000 writes, 3,001 acknowledged offset snapshots, and 1,000/1,000 final offsets** through controller loss and full restart. It committed **672/672** shard-quorum transactions with zero quorum failure, admission rejection, batch failure, or torn-tail truncation, measuring **368.719 writes/s** and **256.974 ms p99**. This is development-machine evidence, not a production SLO. The broker-local mutation lock kept peak end-to-end quorum concurrency at one, and distributed decided-transaction recovery, journal compaction, format-12 rolling qualification, high-cardinality membership/transaction churn, and dedicated-host capacity remain open. Every advertised Metadata version (v4-v12) remains schema-tested, and anonymous idempotent producers bootstrap through every broker.
...
| ---: | --- | --- |
| 1 | Coordinator completion | Format-12 per-shard durability and quorum replication are implemented; distributed resolution of decided transactions, shard-journal checkpoints, finer-grained service mutation, membership/transaction churn at scale, format-12 rolling qualification, and dedicated-host capacity remain |
...
| 3 | Consumer groups | Expand ConsumerGroupHeartbeat beyond v0 and add ConsumerGroupDescribe plus offset administration APIs |
...
- Coordinator ownership is rendezvous-sharded, and format 12 commits changed shards through forced prepare/decision/finalize records on a voter majority without a metadata-log append. The owner publishes only after terminal durability, and monotonic merging preserves disjoint owners' changes. Group and delivery services still share one broker-local mutation lock, so I do not claim linear horizontal scaling.
- A broker that restarts can resume its own prepared or decided shard transaction from local journals. A different owner cannot yet query the distributed decision and finish a transaction whose original coordinator died after majority decision. Per-shard journals also need checkpoint compaction and a tested indefinite-churn disk budget.
- Formats 9–11 retain [bounded controller publication](docs/coordinator-publication.md) for rolling compatibility. Format 12 bypasses that steady-state path. The new format still needs a pinned format-11-to-format-12 rolling/rollback campaign before release.
- OffsetFetch and `read_committed` decisions use [immutable acknowledged views](docs/coordinator-read-isolation.md). They stay on the old complete view during a tentative transaction and switch atomically after acknowledgement. Group, delivery, and checkpoint mutation still use shared broker-local coordination.
...
| `--coordinator-publication-max-requests` | `64` | Maximum compatible shard proposals in one controller publication |
| `--coordinator-publication-max-bytes` | `16777216` | Maximum encoded bytes in one controller publication |
| `--coordinator-publication-pending-requests` | `1024` | Maximum queued plus in-flight controller proposals |
| `--coordinator-publication-pending-bytes` | `67108864` | Maximum estimated retained controller proposal bytes |
| `--coordinator-publication-linger-ms` | `2` | Maximum controller collection delay for compatible proposals |
| `--coordinator-publication-queue-timeout-ms` | `5000` | Queue deadline before publication starts; active proposals wait for the quorum result |
| `--coordinator-quorum-max-inflight` | `256` | Maximum format-12 shard transactions admitted concurrently per broker |
| `--coordinator-quorum-admission-timeout-ms` | `5000` | Maximum wait for a format-12 quorum admission permit |
...
- [Independent coordinator architecture](docs/coordinator-architecture.md)
- [Acknowledged coordinator read isolation](docs/coordinator-read-isolation.md)
- [Consumer group administration](docs/group-administration.md)
