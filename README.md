...
The current release is `1.7.0`. Broker build metadata, Docker labels, the compatibility package, and deployment manifests use the same version. This source release completes dynamic cluster-wide quota reclamation described in the [1.7.0 release notes](docs/releases/1.7.0.md); its Docker Hub tag is not published.

`miladsade96/cascade:1.3.1` remains the latest published Docker Hub image. I build `miladsade96/cascade:1.7.0` locally from this source, but I do not describe that tag as published until the runtime, client, cluster, and zero-finding security gates pass for the exact image and the registry digest is recorded. I do not move `latest` implicitly.
...
The latest [coordinator completion campaign](docs/performance/2026-09-12-coordinator-completion.md) kept 1,000 Kafka consumers resident and preserved 3,000 acknowledged writes plus every final offset through controller failover and full restart. It committed 668/668 shard-quorum transactions with no checkpoint, quorum, admission, or batching failure. The measured phase reached **283.180 writes/s** with **283.373 ms p99** on my Windows development host. This is correctness and regression evidence, not a dedicated-host capacity limit or a controlled comparison with earlier revisions.
...
After implementing exact dynamic quota reclamation, I ran the complete suite on 2026-09-15: **577/577 passed** in **156 seconds**. Focused tests prove that idle capacity can move to a busy broker while concurrent reservations remain inside the configured cluster-wide rate and burst, including controller failover and configuration mismatch on a real three-broker cluster. I record the safety properties and the remaining capacity/soak boundary in the [distributed quota qualification](docs/performance/2026-09-15-distributed-quotas.md).
...
- Coordinator ownership is rendezvous-sharded, and format 12 commits changed shards through forced prepare, vote, certificate, and finalize records on a voter majority without a metadata-log append. A successor resolves in-doubt work from voter decision queries, and conflict-driven reconciliation advances a stale owner before retry. Group and delivery use separate mutation monitors; each service still serializes its own mutable image, so I do not claim linear horizontal scaling.
- Completed per-shard journals compact to forced checkpoint records through atomic replacement, while unresolved prepare/vote/certificate records remain recoverable. I still need a multi-day churn campaign to validate the configured disk budget on production storage.
- Formats 9–11 retain [bounded controller publication](docs/coordinator-publication.md) for rolling compatibility. Format 12 bypasses that steady-state path. The pinned format-11-to-format-12 source campaign passes rollback before activation, downgrade rejection after activation, majority traffic, broker restoration, and exact recovery; an exact published-image pair remains open.
- OffsetFetch and `read_committed` decisions use [immutable acknowledged views](docs/coordinator-read-isolation.md). They stay on the old complete view during a tentative transaction and switch atomically after acknowledgement. Group and delivery checkpoint through independent monitors, while each service still serializes its own mutable image.
...
- At distributed-quota feature level 2, the active controller serializes exact per-principal/class reservations, so a busy broker reclaims idle capacity without exceeding the configured cluster rate or burst. New controller terms cold-start, inconsistent broker settings and ambiguous coordination fail closed, and mixed clusters retain conservative level-1 shares. I still need the long authenticated multi-tenant soak and dedicated-host controller-capacity qualification.
