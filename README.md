...
The current Scala test suite passes **456/456 tests**, covering unit, TCP integration, Kafka-client end-to-end, security, and fault/recovery layers. [Bounded offset batching](docs/offset-batching.md) adds admission limits, atomic validation/read isolation, lifecycle-safe publication, and a fix for OffsetFetch readiness races. Five historical 1,000-group batching campaigns and the current [publication campaign](docs/performance/2026-09-05-coordinator-publication.md) each verified **3,000 writes and 1,000/1,000 final offsets** through controller loss and full restart. The latest million-record regression verified every record at **493,225 produced records/s** and **489,511 consumed records/s**. Linux storage/batching previously passed **51/51** tests, and all five external languages now verify 25/25 records, with Java restart recovery. Shared locks and full-state CPU work still prevent a production-capacity claim. Every advertised Metadata version (v4-v12) remains schema-tested, and anonymous idempotent producers bootstrap through every broker. The separate external-language matrix remains a CI gate:
...
| ---: | --- | --- |
| 1 | Coordinator capacity | Bounded cross-owner publication batching is implemented; independent per-shard consensus beyond the shared metadata quorum, finer-grained service locks, remaining full-state CPU work, membership/transaction churn at scale, and dedicated-host capacity qualification remain |
