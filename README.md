...
The current release is `1.3.0`. Broker build metadata, Docker labels, the compatibility package, and deployment examples use the same version. The published Docker Hub image is `miladsade96/cascade:1.3.0` for Linux/amd64 and includes shard-object storage, bounded offset batching, cached coordinator snapshots, and ownership-aware consumer expiry. I record image qualification, verified publication, and the immutable digest in the [1.3.0 release notes](docs/releases/1.3.0.md). This release does not change `latest` or older image tags.
